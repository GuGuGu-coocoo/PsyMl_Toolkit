...
按研究设计选择，而不是挑得分最高的一种。多选时，每种验证分别排名；列表中第一项是**主要验证**，最终最佳模型只从该验证的结果中产生，其余验证是敏感性分析。
...
“每模型最多候选数”限制组合爆炸；超过上限时使用固定随机种子抽取候选，因此可以复现。“内部折数”只用于参数选择。每一个外部训练折都单独进行内部交叉验证，外部测试折不会参与参数选择。模型比较完成后，获胜模型会在全部可用数据上再做一次内部交叉验证，以确定最终拟合参数；这不会改写外层验证得到的性能估计。
...
结果页先显示主要验证选出的最佳模型和参数，再显示模型—验证组合排行榜。排行榜在**每种验证内部**从 1 开始排名；不同验证产生的分数不能当作同一场竞赛直接排序。
...
Choose validation from the research design, not from whichever score looks best. The first selected validation is primary and determines the final winning model. Additional selections are sensitivity analyses and receive their own rankings.
...
The candidate cap controls combinatorial growth; excess combinations are sampled reproducibly with the fixed seed. Inner folds are used only for parameter selection. Each outer training fold runs its own inner cross-validation, so the outer test fold is never used to choose parameters. After model comparison, the winning model is tuned once more by inner cross-validation over all available rows to obtain its final fitted parameters; this does not alter the outer performance estimate.
...
The result tab shows the best model selected under the primary validation, its parameters and the model–validation leaderboard. Ranks restart within each validation; scores from different validation designs are not one pooled competition.
...
Choisissez la validation selon le plan de recherche, jamais selon le meilleur score observé. La première validation sélectionnée est principale et détermine le modèle final. Les autres sont des analyses de sensibilité avec leur propre classement.
...
La limite de candidats évite l’explosion combinatoire ; les combinaisons excédentaires sont échantillonnées de façon reproductible. Les plis internes servent uniquement au choix des paramètres. Chaque pli externe d’entraînement effectue sa propre validation croisée interne : le pli externe de test ne choisit jamais les paramètres. Après la comparaison, le modèle gagnant est réglé une dernière fois par validation interne sur toutes les lignes disponibles afin d’obtenir ses paramètres finaux ; cela ne modifie pas l’estimation externe des performances.
...
L’onglet de résultats affiche le meilleur modèle selon la validation principale, ses paramètres et le classement modèle–validation. Le rang recommence à 1 dans chaque validation : les scores issus de conceptions différentes ne forment pas une compétition unique.
