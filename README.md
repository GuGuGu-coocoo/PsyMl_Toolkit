...
Phase 0–11 已完成，当前进入 Phase 12：冻结版本化配置/结果接口，扩展 CLI 与本地子进程接口，并建立跨平台打包验证。Legacy 审查已确认旧脚本存在数据泄漏、可移植性与可复现性风险，不应用于新的分析。
...
Phases 0–11 are complete. Phase 12 is now freezing versioned configuration/result interfaces, extending the CLI and local subprocess protocol, and establishing cross-platform packaging checks. The legacy review found leakage, portability and reproducibility risks, so old scripts must not be used for new analyses.
...
Les phases 0–11 sont terminées. La phase 12 fige maintenant les interfaces versionnées de configuration et de résultats, étend la CLI et le protocole de sous-processus local, puis établit les contrôles de packaging multiplateforme. La revue du code historique a identifié des risques de fuite, de portabilité et de reproductibilité ; les anciens scripts ne doivent donc pas servir aux nouvelles analyses.
