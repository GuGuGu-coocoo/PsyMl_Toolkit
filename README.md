...
- A small version label under the application name shows the current version: a standalone package reads its bundled `BUILD.json`, a source run reads `__version__` in `src/psyml/__init__.py` (`pyproject.toml` is dynamic and reads the same constant); the official `0.3.0` displays unchanged, a `0.3.0.dev0` development release displays as `0.3.0-dev`, and the value is not hard-coded.
