...
def stack(file,test_size=0.3,estimators=[(('mlp',MLPClassifier(hidden_layer_sizes=(60,30),activation='tanh')),
    ...
    stack_result_dict = {'Test_size': [test_size],
                       'Estimators':[estimators],
                        'Final_estimator':[final_estimator],
    ...
    for i in range(len(ranks)):
        ...
        stack_result_dict[rank_name] = [ranks[i]]
        
    stack_result = pd.DataFrame(stack_result_dict)
    
    return stack_result
def save_stack(file,estimators=[(('mlp',MLPClassifier(hidden_layer_sizes=(60,30),activation='tanh')),
    ('stack', KNeighborsClassifier(n_neighbors=3)))],
    final_estimator=MLPClassifier(hidden_layer_sizes=(60,30),activation='tanh')):
    file = file
    estimators=estimators
