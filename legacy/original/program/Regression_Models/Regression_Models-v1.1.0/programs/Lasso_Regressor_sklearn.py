# -*- coding: utf-8 -*-
'''
@File    :   Lasso_Regressor_sklearn.py
@Author  :   GuGuGu?coocoo! 
@Time    :   2023/08/20 14:54:45 UTC+08:00
...
def lasso(file, test_size=0.3,alpha=0.1, fit_intercept=True, precompute=False, max_iter=1000, positive=False, selection='cycic', random_state=42):
    file = file
    test_size=test_size
    alpha = alpha
    ...
    # print parameters
    print('Lasso_Regressor_Sklearn',
          '\nFileName:',file,
          '\nRunning the lasso model with parameters:',
    ...
    lasso = Lasso(alpha=alpha, fit_intercept=fit_intercept, precompute=precompute, max_iter=max_iter, positive=positive, selection=selection, random_state=random_state)
    ...
    result = permutation_importance(lasso, X, y, n_repeats=10, random_state=1624)
    ...
    lasso_result = pd.DataFrame(result_dict)

    return lasso_result

def save_lasso(file,alpha=0.1, fit_intercept=True, precompute=False, max_iter=1000, positive=False, selection='cycic', random_state=42):
    file = file
    alpha = alpha
    fit_intercept = fit_intercept
    precompute = precompute
