...
Ce guide concerne la modification, la maintenance et la construction de PsyML. Les chercheurs utilisent l’interface sans outils de développement ni ces commandes. La version du code n’a qu’une seule source maintenue : la constante `__version__` de `src/psyml/__init__.py` (`pyproject.toml` la lit via les métadonnées dynamiques de hatch ; la source actuelle est `0.3.0.dev0`, affichée `0.3.0-dev`). Les versions publiées figurent dans Releases ; la publication actuelle reste **v0.2.0** et ne contient pas les correctifs de la seconde série présents dans ces sources.
...
**L’export natif passe uniquement par `tools/build_native.py`.** Les champs de version macOS/Windows de `gui/export_presets.cfg` (`application/short_version`, `application/version`, `application/file_version`, `application/product_version`) contiennent les jetons `@PSYML_MACOS_VERSION@` / `@PSYML_WINDOWS_VERSION@`, pas des numéros publiables ; un export direct depuis l’éditeur Godot échoue ou écrit de mauvaises versions. `build_native.py` dérive les versions numériques de la constante unique de `src/psyml/__init__.py` le temps d’un export (développement `0.3.0.dev0` → macOS `0.3.0`, Windows `0.3.0.0`) puis restaure le modèle dans un bloc `finally` : ni un succès ni un échec ne laisse de preset modifié. Déclenchez donc toujours l’export natif via ce script plutôt que d’utiliser directement le preset Godot, et n’exécutez pas d’export local avant autorisation d’empaquetage.
