...
`tools/run_gui_tests.py` 逐组运行 12 个 headless GUI 测试（`--group <名称>` 可只跑一组，`--list-groups` 列出组名/脚本/完成标记，`--timeout` 设置每组超时，默认 600 秒）。每组必须同时满足退出码 0、输出含该组 `PSYML_*_OK` 完成标记、且不含 `SCRIPT ERROR` 或 `_FAILURE` 标记才算通过；Godot 可能打印脚本错误后仍以 0 退出，所以只检查退出码不够。失败组的原始输出会完整保留在日志中供诊断。逐组等价命令（如需手工调试）：
...
自动测试通过不等于真实研究设计已验证。少于两个测试样本的 R² 边界测试会有预期警告。CI 在 Windows、macOS、Linux 上运行，使用 `uv sync --locked --group dev --extra explain` 安装锁定环境，因此 SHAP 相关测试会真实执行而不是跳过，并通过 `tools/run_gui_tests.py` 运行全部 12 组 GUI 检查；所有需要 SHAP 的 `uv run` 步骤（`uv run --extra explain pytest -q`、传递 `PSYML_PYTHON` 的步骤与 GUI runner）都显式带 `--extra explain`，避免同步时移除 extra；查看对应提交的 CI 状态，不把本地测试等同于其他平台已通过。`.github/workflows/native-test-build.yml` 的手动 Windows 测试构建同样安装 `explain` 与 `build`，并运行 A/C/D bundle smoke（置换/解释/系数）；它只保留测试产物，不发布 Release，其定义改动也不代表 Windows 原生已验证。
...
Fitted coefficients (FR-005) need no `explain` extra: `tests/test_coefficients.py` covers per-family coefficient/intercept reconstruction, `drop`+`none` identity passthrough, category values/types with underscore labels, removed all-missing column records, `drop_idx_`/per-column encoding mapping, training-dtype source, binary LDA sigmoid=probability and multi-class logistic softmax=probability axis semantics, and that a failed check (tolerance/shape/non-finite) returns `error` and refuses to publish; `tests/test_coefficients_cli.py` covers CLI export, overwrite refusal, unverified and failed-write states; `gui/tests/test_coefficients.gd` covers the results-page coefficient status and open entry, per-output-axis intercept/unit/fit-scope display, export and switching. `gui/tests/test_explanation.gd` adds background-row layout assertions (single line, not a collapsed column).

For manual review, load classification data; use target `target`, group `participant`, predictors `score` and `category`, Decision Tree and Dummy, group K-fold with 3 outer/2 inner folds. Add holdout and select no primary validation. Results should begin with a neutral selector; inspect each validation, switch all UI languages without losing choices, check page versus table scrolling, then run regression. Also check error copying, cancellation and retry. Tiny-sample R² warnings are expected in boundary tests. Automated success is not scientific acceptance. CI runs on Windows, macOS and Linux with `uv sync --locked --group dev --extra explain`, so the SHAP tests really execute instead of being skipped, and runs all 12 GUI groups through `tools/run_gui_tests.py`; every SHAP-dependent `uv run` step (`uv run --extra explain pytest -q`, the `PSYML_PYTHON` step and the GUI runner) passes `--extra explain` explicitly so a sync cannot drop the extra; check the commit's CI results separately for each OS. The manual Windows test build in `.github/workflows/native-test-build.yml` also installs `explain` and `build` and runs the A/C/D bundle smokes (permutation/explanation/coefficients); it only keeps test artifacts, never publishes a release, and changing that workflow definition does not mean Windows native has been verified.
