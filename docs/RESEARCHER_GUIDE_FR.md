...
Version du code documentée : **v0.2.0**. Les versions de l’environnement et des dépendances figurent dans `analysis_manifest.json` pour chaque analyse. Une copie des sources peut contenir des correctifs postérieurs au paquet v0.2.0 et non encore publiés (cette fois : retours humains FR-012–FR-016) ; la version affichée par l’interface vient du `BUILD.json` du paquet autonome ou du `pyproject.toml` des sources, et le titre de ce guide ne permet pas d’en déduire le contenu du paquet téléchargé.
...
**Emplacement des artefacts de la page 4.** La page 4 partage une racine de résultats avec la page 2 : la prédiction écrit `<racine>/prediction/run_<horodatage>_<usec>/predictions.parquet`, l’explication SHAP `explanation/run_*/` et les coefficients `coefficients/run_*/`. Chaque opération fige un nouveau dossier d’exécution à son démarrage et n’écrase jamais de fichier existant ; **Ouvrir le dossier de résultats** pointe vers ce dossier réel. Une racine absente, relative ou non inscriptible produit une erreur au lieu d’un repli vers le dossier de données masqué de l’application ; changer la racine n’affecte que les opérations suivantes et les artefacts terminés restent sur le disque.

### Explication SHAP d'un échantillon (facultatif, FR-004)

Une fois le modèle et les données validés à la page 4, expliquez un échantillon : choisissez un fichier de référence (**background**), la ligne (1, hors en-tête), le nombre de lignes de référence (50 par défaut, 1–100) et les cycles de permutation (**permutation cycles**, 5 par défaut, 1–20) ; en classification, choisissez aussi la classe et son libellé d'origine. Le calcul s'exécute dans un sous-processus annulable, peut être lent au premier lancement et peut être interrompu. Le bloc de résultats affiche une cascade cumulative qui progresse de la **valeur de base** (base value) à la sortie du modèle (sens signé, noms et valeurs d'origine, top N plus une barre « autres N (somme) » alors que le CSV conserve toutes les contributions) avec **Ouvrir l'image en cascade**, **Ouvrir le dossier de résultats** et **Exporter l'explication…**. **Exporter l'explication…** n'écrit que dans un dossier nouveau ou vide et n'écrase jamais de fichier existant (une destination non vide reçoit un sous-dossier unique, et le dossier de résultats courant ne peut pas être la destination ; la commande CLI `explain --output-dir` exige elle aussi un dossier nouveau/vide et refuse `--overwrite`). Les artefacts (par défaut sous `explanation/run_*/`) sont `shap_explanation.json`, `shap_contributions.csv`, `shap_waterfall.png` et `shap_explanation_notes.md`, avec `base + Σφ = sortie choisie` (tolérance 1e-7/1e-6) ; les artefacts terminés restent sur le disque lors d'un changement de ligne/classe/réglages ou à la fermeture.
...
Un paquet autonome et une copie des sources peuvent contenir des correctifs différents. Les étapes ci-dessous servent au re-test des sources ; leur réussite ne vaut pas acceptation humaine.

1. Lancez l’interface depuis la racine des sources (sous macOS, double-cliquez sur `Launch PsyML.command`) ; l’installation des dépendances est décrite dans le [guide de développement](DEVELOPMENT_FR.md). Les paquets autonomes n’incluent pas l’environnement de test.
2. Importez une configuration de classification ou de régression de `examples/quickstart/` et exécutez-la une fois. Une petite étiquette de version doit apparaître sous le nom de l’application : les sources correspondent au `pyproject.toml`, un paquet autonome à son `BUILD.json`.
3. Vérifiez les sorties de la page 4 : prédiction, SHAP et coefficients arrivent dans de nouveaux sous-dossiers `run_*` de `prediction/`, `explanation/` et `coefficients/`, sous la racine partagée avec la page 2 ; aucun fichier existant n’est écrasé et rien n’est écrit dans le dossier de données masqué de l’application.
4. Faites défiler entre une longue page et un petit tableau imbriqué : un geste commencé sur la page continue de la faire défiler au passage sur le tableau, seul un geste commencé sur le tableau fait défiler celui-ci ; une pause d’environ 250 ms démarre un nouveau geste qui peut choisir une autre couche ; un clic sur une liste ou un glissement de barre de défilement juste après un geste doit toujours sélectionner ou déplacer (le verrouillage ne concerne que la molette et le balayage).
5. Pendant l’écriture finale des résultats, l’état doit afficher « Finalisation et écriture des résultats… » avec une barre de progression non pleine ; la page des résultats ne s’ouvre qu’après `completed`.
6. Utilisez **Copier l’erreur complète** pour signaler un problème. Ne créez ni paquet ni PDF de distribution avant la réussite du re-test ; ce guide n’affirme pas que ces fonctions ont passé l’acceptation humaine.
