...
Ce guide décrit le contenu du paquet téléchargé **v0.2.0** publié. Une copie des sources correspond actuellement à la version de développement **0.3.0-dev** (numéro de publication 0.3.0) et contient des correctifs issus des retours humains, pas encore livrés dans un paquet (première série FR-012–FR-016 ; seconde série : complément FR-016 et FR-017–FR-020). La version n’a qu’une seule source maintenue : la constante `__version__` de `src/psyml/__init__.py`. `pyproject.toml` la lit via les métadonnées dynamiques, le `BUILD.json` autonome est généré par `tools/build_native.py` à partir de la même constante, et l’interface affiche la même valeur (`0.3.0.dev0` s’affiche `0.3.0-dev`). Les paquets autonomes publiés et les PDF de distribution restent en v0.2.0 et ne contiennent pas ces correctifs : interface et disposition des sorties peuvent donc différer des sources. Consultez `analysis_manifest.json` dans chaque sortie d’analyse pour les versions d’exécution et de dépendances, et ne déduisez pas le contenu du paquet téléchargé du titre de ce guide.
...
**Aperçu de contrôle des données.** Pour une cible de classification, l’interface affiche le nombre de catégories observées, l’effectif de chacune et sa part parmi les valeurs **non manquantes** de la cible, avec le total de lignes, le dénominateur non manquant et le nombre de manquants. Une cible entièrement manquante ne divise jamais par zéro, et les catégories non observées ainsi que NaN/None ne sont pas des classes réelles. Un indice heuristique signale les **colonnes identifiant/participant suspectées** (mot-clé du nom, ou valeurs entières/texte quasi uniques) avec la raison et le ratio ; c’est une simple mention « à juger », qui ne supprime aucune colonne, ne change aucun rôle et ne bloque jamais l’analyse ; la colonne de groupe peut elle-même être un identifiant. Les effectifs proviennent du mode aperçu local ; l’aperçu par défaut ne renvoie aucune valeur.
...
**Ce que signifient concrètement un découpage aléatoire et un découpage groupé.** Un découpage aléatoire sans colonne de groupe (`holdout`, `k_fold`, `stratified_k_fold`) sépare les lignes au hasard : les enregistrements d’un même participant peuvent se retrouver à la fois en entraînement et en test, on ne peut donc pas affirmer qu’une personne n’apparaît que d’un côté. Un découpage groupé (`holdout` avec colonne de groupe, `group_k_fold`, `stratified_group_k_fold`, `leave_one_group_out`) maintient tout un groupe d’un seul côté **au sein d’un même découpage** ; **entre les plis**, ce même groupe peut basculer de l’autre côté, ce qui est normal en validation croisée groupée et ne rompt pas l’isolement. Ce n’est que lorsque la colonne de groupe est réellement un identifiant de participant qu’on peut lire cela comme « les enregistrements d’une personne ne traversent pas entraînement et test » ; une colonne famille, centre ou lot isole ces unités, pas les individus. **Choisir une colonne de groupe ne suffit pas à isoler les groupes pour un holdout ou un K-fold ordinaire** : l’isolement dépend de la stratégie choisie (tableau ci-dessus). L’interface conserve toujours les noms de méthodes professionnels, ne les transforme pas en préréglages de scénario et ne modifie pas l’algorithme de découpage.
...
| Quel écart à la référence Dummy à plis identiques et où sont les échecs ? | `result_interpretation.json`, `interpretation_baseline_differences.csv`, `result_interpretation.md` | Écarts appariés et variabilité entre plis sous la même validation, le même ensemble de plis et la même métrique ; résumé descriptif, jamais réinjecté dans la sélection ou le réglage |
...
### Interprétation des résultats (écart à la baseline, variabilité et échecs)

Le bloc **Interprétation des résultats** de la page des résultats agrège uniquement les preuves déjà produites par les plis sélectionnés (plis par combinaison, `parameter_search.csv`, `model_comparison.csv`, `validation_summary.csv`) et écrit `result_interpretation.json`, `interpretation_baseline_differences.csv` et `result_interpretation.md` ; il ne réajuste rien, ne resélectionne ni famille ni paramètre et ne réinjecte jamais une comparaison descriptive dans la sélection ou le réglage.

- **Écart à la baseline** : uniquement le `dummy` choisi par l’utilisateur et réussi, avec la même validation, le même ensemble de plis et la même métrique, tous les scores appariés étant finis ; sinon une raison de non-comparabilité explicite est renvoyée, sans inventer d’écart. Les métriques « plus grand est meilleur » utilisent procédure−dummy, MAE/RMSE utilisent dummy−procédure : une valeur positive signifie toujours que la procédure fait mieux ; un pli qui a sélectionné le Dummy lui-même a un écart nul et est indiqué, ce qui est valide.
- **Variabilité entre plis** : définition de résumé existante avec ddof=0, purement descriptive ; un pli unique n’a ni écart-type ni stabilité évaluables, aucun seuil de significativité, fiabilité ou instabilité n’est fixé, un écart de score n’est pas une significativité statistique et l’écart-type inter-plis n’est pas un intervalle de confiance.
- **Échecs en couches** : les candidats de la recherche interne, le modèle+validation externe et la validation/procédure entière sont comptés séparément et restent traçables ; les raisons représentatives sont visibles et les enregistrements complets restent dans `parameter_search.csv`, `warnings.json` et les sous-dossiers en échec.
- **Validations indépendantes** : résumées séparément ; la racine ne fournit qu’un aperçu/index sans comparaison entre validations.
...
**Comment lire la sortie de prédiction (`prediction/run_*/predictions.csv`).** Le fichier conserve les données d’origine et l’ordre des lignes et n’ajoute que les colonnes de prédiction : la régression ajoute `predicted_value`, la classification `predicted_class`, et les classifieurs à probabilités natives `probability_<classe>` dans l’ordre des classes. Les colonnes d’origine (cible comprise) ne sont pas modifiées ; les collisions de noms n’ajoutent un suffixe numérique qu’aux nouvelles colonnes. Une cible présente est conservée sans calcul automatique de métriques de validation externe, de calibration ni d’optimisation de seuil. **Classification et régression :** la classification produit des classes et des probabilités non négatives, non nécessairement calibrées, tandis que la régression produit une valeur continue sans colonne de probabilité. **Ouvrir le dossier des prédictions** ouvre le dossier d’exécution, pas le CSV lui-même. Prédire de nouvelles données ne requiert pas de cible réelle et **n’est pas une validation externe** : celle-ci exige un échantillon indépendant, des résultats réels et un plan d’évaluation adapté. Changer modèle, données ou association efface les anciennes prédictions et relance la vérification ; les fichiers des exécutions précédentes restent sur le disque.

### Explication SHAP d'un échantillon (facultatif, FR-004)

Une fois le modèle et les données validés à la page 4, expliquez un échantillon : choisissez un fichier de référence (**background**), la ligne (1, hors en-tête), le nombre de lignes de référence (50 par défaut, 1–100) et les cycles de permutation (**permutation cycles**, 5 par défaut, 1–20) ; en classification, choisissez aussi la classe et son libellé d'origine. Le calcul s'exécute dans un sous-processus annulable, peut être lent au premier lancement et peut être interrompu. Le bloc de résultats affiche une cascade cumulative qui progresse de la **valeur de base** (base value) à la sortie du modèle (sens signé, noms et valeurs d'origine, top N plus une barre « autres N (somme) » alors que le CSV conserve toutes les contributions) avec seulement **Ouvrir l'image en cascade** et **Ouvrir le dossier de résultats** ; l'interface ne propose plus de copie ni d'export. Les artefacts (par défaut sous `explanation/run_*/`) sont `shap_explanation.json`, `shap_contributions.csv`, `shap_waterfall.png` et `shap_explanation_notes.md`, avec `base + Σφ = sortie choisie` (tolérance 1e-7/1e-6) ; les artefacts terminés restent sur le disque lors d'un changement de ligne/classe/réglages ou à la fermeture. La commande `psyml explain --output-dir` exige toujours un dossier nouveau/vide et refuse `--overwrite`, mais l'interface n'offre plus d'action d'export.

Comment lire ces résultats :

- **Valeur de base :** sortie moyenne du modèle pour l'axe choisi sur les lignes de référence ; toutes les contributions s'y ajoutent.
- **Référence (background) :** données servant à approximer la sortie lorsque la variable prend une valeur de référence ; plus de lignes sont généralement plus stables mais plus lentes, et ce n'est pas une norme de population.
- **Échantillon et classe choisis :** déterminent la ligne et l'axe de sortie expliqués ; la classification exige une classe, la régression n'a qu'un axe.
- **Contributions signées :** de combien chaque variable pousse la sortie au-dessus (positive) ou en dessous (négative) de la valeur de base ; `base + Σφ = sortie choisie`, le CSV listant toutes les contributions alors que l'interface regroupe au-delà du top N.
- **Reconstruction :** l'identité tient dans la tolérance comme contrôle ; elle n'implique pas de mécanisme causal et dépend de la référence et des cycles de permutation.
- **Limites d'approximation :** SHAP **approximatif** par permutations finies, non exact ; le remplacement de la référence ne préserve pas les corrélations ; ni effet causal, ni performance de test externe.

Ce sont des SHAP **approximatifs** par permutations finies : non exacts, non causaux, non une performance de test externe ; le remplacement marginal ne préserve pas les corrélations. La première version couvre la classification `logistic_regression`/`decision_tree`/`random_forest` et la régression `linear_regression`/`ridge`/`lasso`/`elastic_net`/`decision_tree`/`random_forest`, et n'accepte que les modèles enregistrés avec les métadonnées d'export PsyML (`psyml_version`/`fit_scope`) et la structure standard `preprocess`+`model` ; les modèles externes ou à prétraitement personnalisé affichent une raison claire. La prédiction ordinaire reste inchangée. Sans la dépendance `explain`, cette section est indisponible.

Implémentation : [enregistrement](../src/psyml/models/persistence.py), [paramètres effectifs](../src/psyml/models/parameters.py), [prédiction](../src/psyml/prediction.py), [explication](../src/psyml/explanation.py).

### Coefficients et intercepts ajustés (FR-005)

Le bloc **Coefficients et intercepts ajustés** (page 4) ne lit que les paramètres du modèle **déjà ajusté** dans l'**espace de coordonnées prétraité** (après imputation, mise à l'échelle et encodage one-hot) ; aucun réajustement, aucun retour vers les unités brutes. Cette première version couvre la régression `linear_regression`/`ridge`/`lasso`/`elastic_net`/`svr` (`kernel='linear'`) et la classification `logistic_regression`/`lda`/`svm` (`kernel='linear'`, binaire seulement) ; coefficients pairwise du SVC multiclasse, noyaux non linéaires, arbres, KNN, MLP et stacking indiquent une raison d'indisponibilité précise. Avec des données de prédiction compatibles, la prédiction (régression) ou le score de décision (classification) est reconstruit via le même Pipeline avec la tolérance 1e-7/1e-6 et l'état de vérification est affiché ; sans données, l'absence de vérification est explicite. Le bloc affiche l'intercept, l'unité de sortie et la portée d'ajustement pour chaque axe et distingue « aucune donnée de vérification » de « échec de vérification » ; un échec bloque la publication de tout artefact (raison concrète, jamais « extraction terminée », pas d'export). `coefficients.json` consigne les colonnes entièrement manquantes supprimées et leur raison, la correspondance par colonne d'origine, `drop_idx_` et les catégories par colonne ; les dtypes d'entraînement viennent des métadonnées ou sont explicitement inconnus. La classification précise l'axe : logistique binaire = log-odds de `classes_[1]` par rapport à `classes_[0]`, logistique multiclasse = logit softmax par classe, SVC linéaire = marge seulement (ni probabilité ni log-odds) ; libellés, types et indices réels sont conservés. La page 4 ne conserve que **Ouvrir le dossier de résultats** (vers le `coefficients/run_*/` de cette exécution) : plus de copie ni d'export. Une extraction réussie écrit `coefficients.csv`, `coefficients.json` et `coefficients_notes.md` dans ce dossier (JSON en dernier). Une analyse normale écrit aussi ces trois fichiers sous `coefficients/` du dossier d’analyse, marqués `fit_scope=all_analyzed_rows`. Ce sont des paramètres ajustés du modèle final (et non des hyperparamètres), sans p-values, intervalles de confiance, signification, causalité ni effet standardisé défini ; prédiction ordinaire, hyperparamètres et SHAP restent inchangés. CLI équivalente : `psyml coefficients --model … --trust-model [--input …] [--output-dir …]` avec `--check-only`, sans l'extension `explain`.

Comment lire ces résultats :

- **Intercept :** valeur de sortie de référence de chaque axe lorsque toutes les variables transformées valent zéro ; elle suit l'unité de l'axe et ne signifie pas « aucune personne » ni « aucune mesure » dans les données brutes.
- **Unités des variables transformées :** chaque coefficient porte sur une colonne après imputation, mise à l'échelle ou encodage one-hot (par exemple `numeric_score` ou `categorical_category_A`), pas sur l'unité d'origine de la variable ; ne pas reconvertir par unités brutes ni comparer directement les tailles entre modèles.
- **Unité de sortie :** en classification, lire avec l'axe des classes — logistique binaire = log-odds par rapport à la classe de référence, logistique multiclasse = logit softmax par classe, SVC linéaire = marge (score de décision brut, ni probabilité ni log-odds) ; en régression, la sortie `predict` garde l'unité de la cible.
- **Portée d'ajustement (`fit_scope`) :** paramètres ajustés du modèle final sur toutes les lignes analysées (`all_analyzed_rows`), pas d'un pli externe, et non des hyperparamètres.
- **Non vérifié et échec :** sans données de prédiction compatibles, le bloc indique « aucune donnée de vérification (non vérifiée) » ; si la reconstruction dépasse la tolérance, a une mauvaise forme ou n'est pas finie, il indique « échec de vérification » et refuse de publier tout artefact de coefficients.
- **Non causal :** les coefficients décrivent une association sous cet ajustement et cette pénalité ; aucune p-value, intervalle de confiance, signification, causalité ni effet standardisé défini.

Implémentation : [coefficients](../src/psyml/models/coefficients.py).
...
Les figures utilisent les prédictions hors apprentissage de la validation principale ou du sous-dossier indépendant consulté. Holdout n’inclut que le test ; la validation croisée inclut généralement une prédiction externe par observation conservée. Class 1, Class 2, etc. suivent l’ordre de `confusion_matrix.csv` et ne désignent pas une classe positive clinique choisie dans l’interface. Plusieurs figures, ou aucune, peuvent être sélectionnées ; elles sont enregistrées dans `figures/`. Ce ne sont pas des graphiques SHAP, ROC ou d’intervalles de confiance. L’importance des variables n’est exportée que si `permutation_importance` est explicitement activé, sous forme de fichier séparé `interpretations/<validation>/permutation_importance.png` (section suivante).

### Importance par permutation (facultatif, `permutation_importance`)

Si elle est activée à la page 1, pour chaque pli externe de chaque validation, le modèle de pli choisi dans les plis internes est expliqué en permutant chaque variable uniquement sur les lignes de test externes de ce pli, avec la `selection_metric` courante pour mesurer la variation. Les résultats sont écrits sous `interpretations/<validation>/` :

- `permutation_raw.csv` — une ligne par permutation (validation, fold, model_family, variable, repeat, metric, importance, direction, baseline_score, n_rows) ;
- `permutation_folds.csv` — par pli : baseline, `repeats`, `mean`, `repeat_std` (ddof=0), `n_rows` et état/erreur ;
- `permutation_summary.csv` — moyenne équipondérée des moyennes de plis dans une validation (`fold_mean_equal_weight`) et `between_fold_std` (ddof=1), avec le nombre de plis réussis/prévus ;
- `permutation.json` — métrique et sens, seed/répétitions, lignes de test, `model_scope=outer_fold_model`, table d’encodage, limites et causes d’échec ;
- `permutation_importance.png` — figure de classement signée avec ligne zéro.

Lecture : `importance` est signée. Pour MAE/RMSE, une valeur positive signifie une erreur accrue ; pour les autres métriques, une baisse de performance. Les négatifs sont conservés, sans normalisation en pourcentages et sans intervalle de confiance ni pourcentage de R². `repeat_std` décrit la variation entre répétitions dans un pli et `between_fold_std` la variation entre plis ; ne jamais les mélanger, et la valeur inter-plis est vide pour un pli unique. L’écart-type inter-plis est une variabilité descriptive, non une erreur standard ni un intervalle de confiance. Les variables corrélées partagent ou masquent l’attribution et la permutation ligne à ligne ne préserve pas la structure de mesures répétées/de groupe : l’interprétation sur données groupées est plus faible et demande un jugement de conception. Les plis absents ou en échec sont marqués partial/failed sans fabriquer de classement complet ; une validation entièrement en échec ne produit ni table de succès ni figure vide. C’est la sensibilité du modèle de pli à une perturbation de variable sous une métrique, non un effet causal, et non l’explication du modèle final enregistré. Aucun fichier `interpretations/` n’est créé sans activation explicite.
...
| Sortie de prédiction (`prediction/run_*/predictions.csv`) | Conserve données et ordre des lignes et n'ajoute que `predicted_class` ou `predicted_value`, avec `probability_*` natif pour certains classifieurs ; **Ouvrir le dossier des prédictions** ouvre le dossier, pas le CSV |
| Valeur de base et référence (background) | Dans l'explication d'un échantillon : sortie moyenne de l'axe choisi sur les lignes de référence, et l'ensemble de référence lui-même ; ce n'est pas une norme de population |
| Contribution (φ) et reconstruction | De combien chaque variable pousse la sortie au-dessus ou en dessous de la valeur de base ; `base + Σφ = sortie choisie` se vérifie, sans être causal |
| Intercept | Sortie de référence lorsque toutes les variables transformées valent zéro, dans l'espace prétraité, pas en unités brutes |
| Variable transformée | Colonne après imputation, mise à l'échelle ou encodage one-hot ; les coefficients s'y rapportent et ne se convertissent pas par unités brutes |
| log-odds / marge | Unité des coefficients de classification : log-odds (logistique binaire), logit softmax (multiclasse), marge (SVC linéaire) ; la marge n'est ni probabilité ni log-odds |
| Portée d'ajustement (`fit_scope`) | À quel ajustement se rapportent les coefficients ; `all_analyzed_rows` = modèle final sur toutes les lignes analysées, pas un pli externe |
...
## Étapes de re-test depuis les sources

Un paquet autonome et une copie des sources peuvent contenir des correctifs différents. Les cinq points ci-dessous servent au re-test des sources ; leur réussite ne vaut pas acceptation humaine. Ne créez ni paquet ni PDF de distribution avant la réussite du re-test ; ce guide n'affirme pas que ces fonctions ont passé l'acceptation humaine.
...
2. **Étiquette de version :** une petite étiquette sous le nom de l'application doit afficher `0.3.0-dev` (développement ; la publication sera `0.3.0`). La source unique des sources est `__version__` dans `src/psyml/__init__.py` (`pyproject.toml` est dynamique) ; un paquet autonome lit son `BUILD.json`, généré à partir de la même constante, et `0.3.0.dev0` s'affiche `0.3.0-dev`.
...
4. **La page 4 ouvre, n'exporte pas :** les trois blocs ne proposent que **Ouvrir le dossier de résultats** (prédiction : **Ouvrir le dossier des prédictions**, qui ouvre le dossier d'exécution, pas le CSV) et **Ouvrir l'image en cascade** ; aucune copie ni export. La sortie de prédiction est un `predictions.csv` directement ouvrable ; plus de `predictions.parquet`.
