...
Une fois le modèle et les données validés à la page 4, expliquez un échantillon : choisissez un fichier de référence, la ligne (1, hors en-tête), le nombre de lignes de référence (50 par défaut, 1–100) et les cycles de permutation (5 par défaut, 1–20) ; en classification, choisissez aussi la classe et son libellé d'origine. Le calcul s'exécute dans un sous-processus annulable, peut être lent au premier lancement et peut être interrompu. Le bloc de résultats affiche une cascade cumulative qui progresse de la valeur de base à la sortie du modèle (sens signé, noms et valeurs d'origine, top N plus une barre « autres N (somme) » alors que le CSV conserve toutes les contributions) avec **Ouvrir l'image en cascade**, **Ouvrir le dossier de résultats** et **Exporter l'explication…**. **Exporter l'explication…** n'écrit que dans un dossier nouveau ou vide et n'écrase jamais de fichier existant (une destination non vide reçoit un sous-dossier unique, et le dossier de résultats courant ne peut pas être la destination ; la commande CLI `explain --output-dir` exige elle aussi un dossier nouveau/vide et refuse `--overwrite`). Les artefacts sont `shap_explanation.json`, `shap_contributions.csv`, `shap_waterfall.png` et `shap_explanation_notes.md`, avec `base + Σφ = sortie choisie` (tolérance 1e-7/1e-6) ; les artefacts terminés restent sur le disque lors d'un changement de ligne/classe/réglages ou à la fermeture.
