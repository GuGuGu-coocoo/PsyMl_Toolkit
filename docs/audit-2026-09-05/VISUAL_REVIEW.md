...
**最终版本更新：**用户随后选择完整嵌套家族/参数选择。本页的30行夹具前后图保留为第一轮视觉修改证据；最终界面已将“最佳”改为“最终拟合”，明确主指标评价完整选择流程、排行榜仅供探索，并区分两种缺失删除提示。最后一轮已通过 Computer Use 实际操作原生 PsyML 窗口，补测了文件选择器、键盘、缩放、终止与错误恢复；原来的桌面接管限制已解决。持续按下的单帧、屏幕阅读器和跨平台仍未验收。完整范围见 [实际 GUI 验收记录](CUA_ACCEPTANCE.md)。

| 当前四页（48行指南数据） | 最终截图 |
|---|---|
| 数据与变量 | [中文](cua-final/01-zh_CN-page1.png) |
| 分析设置 | [中文](cua-final/03-zh_CN-page2.png) · [参数区](cua-final/02-zh_CN-page2.png) |
| 检查与运行 | [中文](cua-final/04-zh_CN-page3.png) |
| 结果 | [中文](cua-final/11-zh_CN-page4.png) · [英文](cua-final/12-en-page4.png) · [法文](cua-final/13-fr-page4.png) |
...
中文前后图均为 1180×800 的真实 Godot 视口，同一30行仓库合成夹具、Decision Tree + Dummy、两种分组验证和相同有限搜索。输出目录按每次运行生成新路径，因此路径不同；第一轮增加的参数与风险说明属于正确性修复。截图是直接从正在运行的视口保存，未经图片编辑。
...
前后总览可点击上表对比；下面示例展示第一轮结果页：

![第一轮中文结果页](after/zh/04-results.png)
