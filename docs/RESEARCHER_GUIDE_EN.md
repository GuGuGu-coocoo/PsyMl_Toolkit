...
Applies to code version **v0.2.0**. See `analysis_manifest.json` in each analysis output for runtime and dependency versions. A source checkout may contain fixes made after the v0.2.0 download package and not yet released with it (this round: human feedback FR-012–FR-016); the interface version comes from the standalone package's `BUILD.json` or from the source `pyproject.toml`, so do not infer the download package's contents from this guide's title.
...
A standalone package and a source checkout can contain different fixes. The steps below are for re-testing a source checkout; passing them is not human acceptance.

1. Start the interface from the source checkout root (on macOS you can double-click `Launch PsyML.command`); dependency installation is in the [developer guide](DEVELOPMENT_EN.md). Standalone packages do not include the developer test environment.
2. Import the classification or regression configuration from `examples/quickstart/` and run it once. A small version label should appear under the application name: the source run matches `pyproject.toml`, a standalone package matches its bundled `BUILD.json`.
3. Check page-4 outputs: prediction, SHAP and coefficients land in new `run_*` subdirectories of `prediction/`, `explanation/` and `coefficients/` under the result root shared with page 2; existing files are not overwritten and nothing is written to the hidden application-data folder.
4. Scroll between a long page and a nested small table: when a gesture starts on the page it keeps scrolling the page past the table, and only a gesture that starts on the table scrolls that table; a pause of about 250 ms starts a new gesture that may choose a different layer; immediately clicking a list or dragging a scrollbar after a scroll gesture must still select or move (the lock only affects wheel/swipe scrolling).
5. During final result writing the status should read "Finalizing and writing results…" with the progress bar not yet full; the result page opens only after `completed`.
6. Use **Copy full error** when reporting a problem. Do not package or generate distribution PDFs before re-testing passes; this guide does not claim that these features passed human acceptance.
