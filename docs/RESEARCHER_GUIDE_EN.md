...
This guide describes what the released **v0.2.0** download package contains. A source checkout is currently the development version **0.3.0-dev** (release number 0.3.0) with human-feedback fixes not yet shipped with a package (first round FR-012–FR-016; second round FR-016 supplement and FR-017–FR-020). There is one maintained version source: the `__version__` constant in `src/psyml/__init__.py`. `pyproject.toml` reads it through dynamic metadata, the standalone `BUILD.json` is generated from the same constant by `tools/build_native.py`, and the interface shows the same value (a `0.3.0.dev0` development release displays as `0.3.0-dev`). The released standalone packages and distribution PDFs remain v0.2.0 and do not contain these fixes, so their interface and output layout may differ from a source checkout. Check `analysis_manifest.json` in each analysis output for runtime and dependency versions, and do not infer the download package's contents from this guide's title.
...
A standalone package and a source checkout can contain different fixes. The five points below are for re-testing a source checkout; passing them is not human acceptance. Do not package or generate distribution PDFs before re-testing passes; this guide does not claim that these features passed human acceptance.
...
2. **Version label:** a small label under the application name should read `0.3.0-dev` (development; the release will be `0.3.0`). A source run's single source is `__version__` in `src/psyml/__init__.py` (`pyproject.toml` is dynamic); a standalone package reads its bundled `BUILD.json`, generated from that same constant, and `0.3.0.dev0` displays as `0.3.0-dev`.
